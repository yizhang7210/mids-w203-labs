...
significance level of $0.05$, representing a non-significant result.

This means that we failed to reject the null hypothesis, i.e. we do not have
sufficient evidence to believe Democrat voter's level of support for Biden and Harris
are different given the data and a 5% significance level.
...
so the effect size is indeed very small.

``` {r echo=FALSE}
feeling_diffs = democraticDf$Biden - democraticDf$Harris
percentage_equal <- length(feeling_diffs[feeling_diffs == 0]) / length(feeling_diffs)
percentage_within_10 <- length(feeling_diffs[abs(feeling_diffs) <= 10]) / length(feeling_diffs)

paste(round(percentage_equal, 2), "% of Democratic voters gave Biden and Harris the same score.")
paste(round(percentage_within_10, 2), "% of Democratic voters gave Biden and Harris scores within 10 points.")
...
hist(democraticDf$Harris - democraticDf$Biden, breaks=20, main="Voter Feelings - Difference", xlab = "Difference in Feeling Score", ylab = "Number of people")
```
