...
The results we use from this survey contain information from 8,280 pre-election 
interviews with U.S. citizens of voting age. 
The ANES survey fits well in our research question. The 
...
We use `V201018` to select Democratic voters. We use `V201151` and `V201153`
...
As no legal data could identify who a party registrant voted for, we assume that the registrant voted for the candidate from their registered party (i.e. A voter registered as a Democrat likely voted for the Democratic candidate, Joe Biden). We considered using the  
...
support of Democrats for Biden ($\mu_B$) and Harris ($\mu_H$) are the same. The *alternative hypothesis* is that they are not equal.
...
significance level of $\alpha = 0.05$, therefore we fail to reject the null hypothesis.
...
On a practical level, we can see that the absolute value of the mean difference 
is fairly small ($0.71$), less than 1 point on the $0-100$ scale.
