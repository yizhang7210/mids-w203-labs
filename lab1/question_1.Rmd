...
unprecedented. Occurring during a global pandemic with political 
polarization at an all-time high and the most diverse candidate pool in U.S. 
history, the 2020 election posed new challenges and opportunities for 
...
Studies (ANES) 2020 Time Series Study. Understanding age distributions could help politicians
...
Republicans, so we remove non or independent,
other parties as well as other non-answers, i.e. "inapplicable," "don't know," and
"refused" to respond.

Similarly, for `SUMMARY: RESPONDENT AGE`, we remove people who refused to answer.

After these cleanup operations, we are left with only 3,074 observations to work with.
...
``` {r echo=FALSE, fig.cap="Distributions of party affiliation and age within the ANES data set", fig.pos='!h', out.extra=''}
par(mfrow=c(1, 2))
...
The unpaired two-sample t-test is the most appropriate to answer this question.
...
unpaired t-test which requires the following assumptions to be true:
...
given the data and a 5% significance level. In addition, a 95% confidence interval for the difference of mean between Democrats and Republicans $\mu_D-\mu_R$ is (-4.5312, -2.0965). Meaning out of all confidence intervals constructed, 95% of the time the interval will contain the true mean.
...
```{r, echo = FALSE, warning=FALSE, message=FALSE, fig.cap="The distribution of age differs for each political party we analyze. The dotted lines represent the average age in each respective political party. Average age of Democrats surveyed is 52.5. Average age of Republicans surveyed is 55.9.", fig.pos='!h', out.extra=''}
